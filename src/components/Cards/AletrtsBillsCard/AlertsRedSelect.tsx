...
const AlertsRedSelect = ({ bill }: any) => {
    const [selectedTabIdx, setSelectedTabIdx] = useState(0);

    useEffect(() => {
        let count = -1
        bill?.BillStages[1]['SenateBillStages'].map((stage: any) => {
    ...
    return (
        <div className="grow">
            ...
            >
                {tabs.map((tab, i) => (
                    <div
                        ...
                        className={`
                         ${i <= selectedTabIdx
                                ? " !bg-[#7a041d] !text-white focus:outline-none "
                                : "bg-theme-gray-100"
            ...
            <div className={`bg-[#7a041d] h-1 rounded-md ${selectedTabIdx===0 ? "w-[33%]" : selectedTabIdx===1 ? "w-[66%]" :  "w-full"}`}></div>
        </div>
    );
};
export default AlertsRedSelect;